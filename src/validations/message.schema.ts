...
export const messageTypeSchema = z.enum(["text", "image", "video", "file"], {
  error: "Message must be a type of text, image, video or file",
});
...
export const messageSchema = z.object({
  ...
  attachmentUrl: z
    .union([z.url(), z.literal("")], {
      error: "Attachment url must be an url or empty",
    })
    .optional(),
  messageType: messageTypeSchema,
});

export const messageStateType = z.enum(["sent", "delivered", "seen"], {
  error: "Message state must be a type of sent, delivered or seen",
});
